...
class Message(db.Model):
    id = PrimaryKeyField()
    user_id = ForeignKeyField(User, db_column='user_id' , related_name='userids')
    content = TextField()
...
class SipBuddies(db.Model):
    ...
    disallow = CharField()
    dtmfmode = CharField()
    fromdomain = CharField(null=True)
    fromuser = CharField(null=True)
    fullcontact = CharField(null=True)
    host = CharField(index=True)
    id_cc_card = IntegerField()
    incominglimit = CharField()
    insecure = CharField(null=True)
    ipaddr = CharField(index=True)
    language = CharField(null=True)
    lastms = CharField(null=True)
    mailbox = CharField(null=True)
    mask = CharField(null=True)
    maxcallbitrate = CharField()
    md5secret = CharField(null=True)
    mohsuggest = CharField()
    musicclass = CharField()
    musiconhold = CharField(null=True)
    name = CharField(unique=True)
