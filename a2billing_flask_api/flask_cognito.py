...
class CognitoAuthError(Exception):
    ...
    def __repr__(self):
        return "CognitoAuthError: {self.error}"
...
class CognitoAuth(object):
    ...
    def get_token(self):
        ...
        if parts[0].lower() != auth_header_prefix.lower():
            raise CognitoAuthError('Invalid Cognito JWT header', 'Unsupported authorization type. Header prefix "{parts[0].lower()}" does not match "{auth_header_prefix.lower()}"')
        elif len(parts) == 1:
...
def _cognito_auth_required():
    ...
    if token is None:
        auth_header_name = _cog.jwt_header_name
        auth_header_prefix = _cog.jwt_header_prefix
        raise CognitoAuthError('Authorization Required', 'Request does not contain a well-formed access token in the "{auth_header_name}" header beginning with "{auth_header_prefix}"')
